...
dat <- metadat::dat.baskerville2012
```
...
- You need to gather evidence to make an informed decision about the e**fficacy of an intervention**
...
- **Study design:** Which study designs are eligible?

  - Randomized Controlled Trials (RCTs), pre-post, etc.

  <!-- -->

  - Different study designs may require **different effect-size calculations**
:::
...
## Calculating effect size using `meta` {#sec-overview}

::: {#meta-fig5}
![Overview of functions for calculating different types of effect sizes using the `meta` package. Obtained from Harrer et al. [-@harrer2021]](images/fig5.png){#fig-fig1}
:::
...
- The weight of an individual study ($w_k$) is calculated as follows: $$w_k = \frac{1}{SE^2_k}$$
...
The variance of the distribution of effect sizes is denoted by $\tau_2$ and represents between-study heterogeneity
...
- This second source of variation allows us to quantidy between-study heterogeneity
...
- The **`metadat`** package provides a collection of datasets for conducting meta-analyses

- We will use the **`dat.baskerville2012`** dataset
...
> The intercept was `r round(egger$intercept, 2)`, which was significantly different from zero, $t$ = `r round(egger$statistic, 2)`, $p$ = `r format.pval(egger$pval, digits = 3, scientific = FALSE)`. This indicates funnel plot asymmetry, consistent with small-study effects. However, this pattern does not necessarily indicate publication bias, as funnel plot asymmetry can have other causes.
...
- All bias-detection tests have **limitations** and should not be interpreted in isolation.
...
Using SEM instead of SD changes the effect size from about 0.46 to 2.92, making the study appear to have an enormous effect
